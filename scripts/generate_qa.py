...
"""
import json, argparse, hashlib, re, logging
from pathlib import Path

_log = logging.getLogger(__name__)

# ─── 矿床类型映射（键名与 deepseek_extract.py 词表保持一致）─────────────────
DEPOSIT_NAMES = {
    # 斑岩型
    "PORPHYRY-CU":      "porphyry copper",
    "PORPHYRY-CU-AU":   "porphyry Cu-Au",
    "PORPHYRY-CU-MO":   "porphyry Cu-Mo",
    "PORPHYRY-MO":      "porphyry molybdenum",
    "PORPHYRY-AU":      "porphyry gold",
    "PORPHYRY-SN":      "porphyry tin",
    "PORPHYRY-W":       "porphyry tungsten",
    "PORPHYRY-SKARN":   "porphyry-skarn",
    "PORPHYRY":         "porphyry",
    # 浅成热液
    "EPITHERMAL-HS":    "high-sulfidation epithermal",
    "EPITHERMAL-IS":    "intermediate-sulfidation epithermal",
    "EPITHERMAL-LS":    "low-sulfidation epithermal",
    "EPITHERMAL-AU":    "epithermal gold",
    "EPITHERMAL-AG":    "epithermal silver",
    "EPITHERMAL-AG-AU": "epithermal Ag-Au",
    "EPITHERMAL":       "epithermal",
    # 造山型金矿
    "OROG-AU":          "orogenic gold",
    "OROG-AU-PALEO":    "paleoplacer gold",
    "CARLIN-AU":        "Carlin-type gold",
    "JACUTINGA-AU":     "Jacutinga-type gold",
    # IOCG & 矽卡岩
    "IOCG":             "iron oxide copper-gold (IOCG)",
    "KIRUNA-FE":        "Kiruna-type iron-apatite",
    "SKARN-CU-AU":      "Cu-Au skarn",
    "SKARN-CU":         "copper skarn",
    "SKARN-PB-ZN":      "Pb-Zn skarn",
    "SKARN-FE":         "iron skarn",
    "SKARN-W":          "tungsten skarn",
    "SKARN-W-SN":       "W-Sn skarn",
    "SKARN-SN":         "tin skarn",
    "SKARN-AU":         "gold skarn",
    "SKARN-MN":         "manganese skarn",
    "SKARN":            "skarn",
    # 块状硫化物
    "VMS":              "volcanogenic massive sulfide (VMS)",
    "SMS":              "seafloor massive sulfide (SMS)",
    "SEDEX":            "sedimentary exhalative (SEDEX)",
    "MVT":              "Mississippi Valley-type (MVT)",
    "IRISH-PB-ZN":      "Irish-type Pb-Zn",
    # 岩浆硫化物
    "NI-CU-PGE":        "magmatic Ni-Cu-PGE",
    "NI-CU":            "magmatic Ni-Cu",
    "NI-LATERITE":      "nickel laterite",
    "PGE-REEF":         "PGE reef",
    "PGE-CR":           "PGE-chromitite",
    "CR-OPHIOLITE":     "podiform chromitite",
    "CR-STRATIFORM":    "stratiform chromitite",
    # 碳酸岩/伟晶岩
    "CARBONATITE-REE":  "carbonatite REE",
    "CARBONATITE-NB":   "carbonatite niobium",
    "CARBONATITE-P":    "carbonatite phosphate",
    "CARBONATITE":      "carbonatite",
    "PEGMATITE-LCT":    "LCT pegmatite",
    "PEGMATITE-NYF":    "NYF pegmatite",
    "PEGMATITE-REE":    "REE pegmatite",
    "PEGMATITE":        "pegmatite",
    # 铀矿
    "U-UNCONFORMITY":   "unconformity-type uranium",
    "U-SANDSTONE":      "sandstone uranium",
    "U-VEIN":           "vein uranium",
    # 沉积型
    "BIF-FE":           "BIF iron",
    "COAL":             "coal",
    "SEDIMENTARY-MN":   "sedimentary manganese",
    # 脉型
    "VEIN-AU":          "gold vein",
    "VEIN-AG":          "silver vein",
    "VEIN-CU":          "copper vein",
    "VEIN-PB-ZN":       "Pb-Zn vein",
    "GREISEN-W-SN":     "greisen W-Sn",
    "GREISEN-W":        "greisen tungsten",
    # 兜底
...
# 矿床类型→成因流体描述（键名对齐词表）
FLUID_DESC = {
    "PORPHYRY-CU":      "magmatic-hydrothermal fluids exsolved from a cooling felsic intrusion",
    "PORPHYRY-CU-AU":   "magmatic-hydrothermal fluids derived from oxidized, sulfur-rich magmas",
    "PORPHYRY-CU-MO":   "magmatic-hydrothermal fluids exsolved from a cooling felsic intrusion",
    "PORPHYRY-MO":      "magmatic-hydrothermal fluids exsolved from a highly fractionated felsic intrusion",
    "PORPHYRY-AU":      "magmatic-hydrothermal fluids derived from oxidized arc magmas",
    "PORPHYRY":         "magmatic-hydrothermal fluids exsolved from a cooling felsic intrusion",
    ...
    "OROG-AU-PALEO":    "placer-forming fluvial and alluvial processes in a paleoarchean setting",
    "CARLIN-AU":        "deeply-sourced fluids ascending along high-angle structures",
    ...
    "IRISH-PB-ZN":      "basinal brines ascending through carbonate-hosted fracture networks",
    "EPITHERMAL-HS":    "acidic magmatic vapors condensing in the shallow volcanic environment",
    "EPITHERMAL-IS":    "near-neutral pH magmatic-meteoric mixed fluids",
    "EPITHERMAL-LS":    "near-neutral pH fluids of mixed meteoric-magmatic origin ascending along faults",
    "EPITHERMAL-AU":    "low-temperature hydrothermal fluids in a near-surface volcanic environment",
    "EPITHERMAL":       "low-temperature hydrothermal fluids in a near-surface volcanic environment",
    "SKARN-CU-AU":      "magmatic-hydrothermal fluids reacting with carbonate host rocks at the intrusion contact",
    "SKARN":            "magmatic-hydrothermal fluids reacting with carbonate host rocks at the intrusion contact",
    "NI-CU-PGE":        "sulfide-saturated mafic-ultramafic magmas segregating immiscible sulfide liquids",
    "NI-CU":            "sulfide-saturated mafic-ultramafic magmas segregating immiscible sulfide liquids",
    "BIF-FE":           "chemically precipitated iron from Archean-Proterozoic seawater",
}

# 矿床类型→构造控矿描述（键名对齐词表）
STRUCT_DESC = {
    "PORPHYRY-CU":    "Stockworks and fracture networks developed in the brittle carapace above the causative pluton, while regional faults provided distal fluid pathways.",
    "PORPHYRY-CU-AU": "Fracture-controlled stockwork veining concentrated in the apical zone of the porphyry intrusion.",
    "PORPHYRY-CU-MO": "Stockworks and fracture networks developed in the brittle carapace above the causative pluton, while regional faults provided distal fluid pathways.",
    "PORPHYRY-MO":    "Stockworks and breccia zones above the causative highly-evolved granite intrusion.",
    "PORPHYRY":       "Stockworks and fracture networks developed in the brittle carapace above the causative pluton.",
    ...
    "CARLIN-AU":      "High-angle feeder faults intersecting reactive carbonate stratigraphy created the most favorable ore traps.",
    "EPITHERMAL-HS":  "Subvertical fracture zones provided conduits for ascending magmatic vapors to react with groundwater.",
    "EPITHERMAL-LS":  "Dilational zones in normal and strike-slip faults provided open-space for quartz-adularia vein formation.",
    "EPITHERMAL-IS":  "Fault-controlled vein systems and breccias hosted mineralization at intermediate crustal levels.",
    "EPITHERMAL":     "Structural conduits in volcanic terranes focused hydrothermal flow to near-surface depositional sites.",
    "SKARN-CU-AU":    "Ore localization was controlled by the geometry of the intrusion-carbonate contact and cross-cutting faults.",
    "SKARN":          "Ore localization was controlled by the geometry of the intrusion-carbonate contact and cross-cutting faults.",
    "NI-CU-PGE":      "Structural embayments and dynamic flow constrictions in magma conduits trapped dense sulfide liquids.",
    "NI-CU":          "Structural embayments and dynamic flow constrictions in magma conduits trapped dense sulfide liquids.",
    "BIF-FE":         "Fold hinges and shear zones within the BIF provided structural preparation and enhanced permeability.",
}
...
TIER_A_DIMS = {
    'spatial_location', 'spatial_model', 'structural_control', 'temporal_spatial',
    'host_rock_relation', 'alteration_zonation', 'mineral_assemblage',
    'exploration', 'comparative',
}


def _adapt_schema(record: dict) -> dict:
    """
    将新版 deepseek_extract schema 适配到原版 generate_qa 期望的字段格式。
    不修改原始对象，返回副本。

    新 schema:
      commodities: {"primary": [...], "byproduct": [...], "trace": [...]}
      minerals:    {"ore_minerals": [...], "gangue_minerals": [...], "alteration_minerals": [...]}
      ages:        [{"age_ma": float, ...}]
      countries:   ["Australia", ...]  (plural)

    原版期望:
      commodities:        ["Au", "Ag"]  (flat list)
      minerals_mentioned: ["pyrite", ...]  (flat list)
      ages_ma:            [250.0, ...]  (flat list)
      country:            ["Australia", ...]  (original key)
    """
    r = dict(record)

    # commodities: dict → flat list
    comm = r.get("commodities")
    if isinstance(comm, dict):
        primary = comm.get("primary") or []
        byproduct = comm.get("byproduct") or []
        r["commodities"] = [x for x in primary + byproduct if x]

    # minerals: dict → minerals_mentioned flat list
    mins = r.get("minerals")
    if isinstance(mins, dict):
        ore = mins.get("ore_minerals") or []
        gangue = mins.get("gangue_minerals") or []
        alt = mins.get("alteration_minerals") or []
        r["minerals_mentioned"] = [x for x in ore + gangue + alt if x]
    elif "minerals_mentioned" not in r:
        r["minerals_mentioned"] = []

    # ages: list of dicts → flat float list
    ages = r.get("ages")
    if ages and isinstance(ages, list) and isinstance(ages[0], dict):
        r["ages_ma"] = [a["age_ma"] for a in ages if a.get("age_ma") is not None]

    # countries → country（原版用 country 字段）
    if "country" not in r and "countries" in r:
        r["country"] = r["countries"]

    return r


def generate_qa_for_record(record):
    record = _adapt_schema(record)  # 新旧 schema 自动适配
    results = []
...
def _normalize_record(r):
    """统一 commodities / list-type 字段，兼容旧 list 和新 dict schema。"""
    r = dict(r)
    ...
    if isinstance(c, dict):
        ...
        r['commodities'] = flat if flat else None

    # minerals: {ore_minerals, gangue_minerals, alteration_minerals} → minerals_mentioned flat list
    if 'minerals_mentioned' not in r or not r['minerals_mentioned']:
        m = r.get('minerals')
        if isinstance(m, dict):
            flat_m = []
            for sub in ('ore_minerals', 'gangue_minerals', 'alteration_minerals'):
                flat_m.extend(x for x in (m.get(sub) or []) if x)
            r['minerals_mentioned'] = flat_m if flat_m else None

    # ages: [{age_ma: float, ...}] → ages_ma flat list of floats（供模板 min/max 运算）
    if 'ages_ma' not in r or not r['ages_ma']:
        ages_raw = r.get('ages')
        if isinstance(ages_raw, list):
            flat_a = []
            for a in ages_raw:
                v = a.get('age_ma') if isinstance(a, dict) else a
                try:
                    flat_a.append(float(v))
                except (TypeError, ValueError):
                    pass
            r['ages_ma'] = flat_a if flat_a else None
...
def main():
    ...
    _log.info(f"输入: {len(data)} 条, {len(TEMPLATES)} 模板")
    ...
    for record in data:
        spatial_fields = ["host_rocks", "structural_controls", "alteration",
                         ...
                         "ages", "commodities", "deposit_type"]
        filled = sum(1 for f in spatial_fields if record.get(f))
        if filled < args.min_fields:
            skipped += 1
            continue
        all_qa.extend(generate_qa_for_record(_normalize_record(record)))

    with open(Path(args.output), "w", encoding="utf-8") as f:
        for qa in all_qa:
            f.write(json.dumps(qa, ensure_ascii=False) + "\n")

    _log.info(f"输出: {len(all_qa)} 条 → {args.output}")
    _log.info(f"  跳过: {skipped} 条 | 覆盖: {len(data)-skipped} 篇")
