...
async def run(corpus_root: str, output_dir: str, max_papers: int = 0,
    ...
    if os.path.exists(done_file):
        done_meta = json.load(open(done_file))
    ...
    if prescreen and len(todo) > 5:
        ...
        before = len(todo)
        todo = [p for p, keep in zip(todo, ps_results) if keep]
        skipped = before - len(todo)
        print(f"  → 保留 {len(todo)} 篇, 跳过 {skipped} 篇非矿床论文")
    ...
    for batch_start in range(0, len(todo), batch_size):
        ...
        json.dump([r for r in batch_results if r], open(chunk_file, 'w'), ensure_ascii=False, indent=1)
        json.dump({'prompt_hash': current_hash, 'ids': list(done_ids)}, open(done_file, 'w'))

    # P2: 保存 usage.json
    usage_file = os.path.join(output_dir, 'usage.json')
    json.dump(usage_acc, open(usage_file, 'w'), indent=2)
    ...
    # 读取所有 chunk 文件合并
    all_data = []
    for f in sorted(glob.glob(os.path.join(output_dir, 'results_*.json'))):
    ...
    # 写入结果
    json.dump(gated_data, open(all_results_file, 'w'), ensure_ascii=False, indent=1)
    json.dump(trusted, open(os.path.join(output_dir, 'trusted.json'), 'w'), ensure_ascii=False, indent=1)
    if quarantine:
        json.dump(quarantine, open(os.path.join(output_dir, 'quarantine.json'), 'w'), ensure_ascii=False, indent=1)
