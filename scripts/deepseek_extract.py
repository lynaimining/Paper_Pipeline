...
async def run(corpus_root: str, output_dir: str, max_papers: int = 0,
    ...
    # P0-A: 前置筛选——快速判断是否矿床论文（分批并发，避免 42K 任务同时挂起）
    if prescreen and len(todo) > 5:
        print(f"\n[P0-A] 前置筛选: 并发检查 {len(todo)} 篇是否矿床论文...")
        ps_batch = 500  # 每批 500，分批 gather
        ps_results = []
        for i in range(0, len(todo), ps_batch):
            ...
            ps_results.extend(chunk_results)
            done_so_far = min(i + ps_batch, len(todo))
            print(f"  P0-A 进度: {done_so_far}/{len(todo)}", flush=True)
        before = len(todo)
        original_todo = todo
    ...
    for batch_start in range(0, len(todo), batch_size):
        ...
        # 收集成功的；None 表示 extract_one 三次重试全败，记录到 failed 列表
        failed_ids = []
        for p, r in zip(batch, batch_results):
            if r is not None:
                ...
                print(f"  [LOST] {p['paper_id']}: 抽取失败（3次重试后返回 None），已记录", file=sys.stderr)
