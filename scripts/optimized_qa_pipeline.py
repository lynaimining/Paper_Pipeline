...
async def run_optimized_pipeline(
    ...
    concurrency: int = 20,
    chunk_size: int = 500,
    paper_ids: set | None = None,   # 只处理这些 paper_id，None 表示全部
):
    """
    运行完整的优化pipeline
    ...
    # paper_ids 过滤：只处理指定论文的图像，避免传入大文件时处理全量
    if paper_ids:
        before = len(caption_images)
        caption_images = [img for img in caption_images if img.get('paper_id') in paper_ids]
        print(f"  → paper_ids 过滤: {before} → {len(caption_images)} 张（{len(paper_ids)} 篇论文）")
    ...
    # ── Checkpoint/Resume ─────────────────────────────────────────────────────
    ckpt_path = output_path.replace('.jsonl', '_ckpt.json')
    text_demoted_path = output_path.replace('.jsonl', '_text_demoted.jsonl')
    done_image_paths: set = set()

    if os.path.exists(ckpt_path):
        with open(ckpt_path) as f:
            ckpt = json.load(f)
        done_image_paths = set(ckpt.get('done_image_paths', []))
        print(f"  ↩️  从 checkpoint 恢复: 已处理 {len(done_image_paths)} 张")
    else:
        open(output_path, 'w').close()
        open(text_demoted_path, 'w').close()

    remaining = [img for img in caption_images if img['image_path'] not in done_image_paths]
    print(f"  待处理: {len(remaining)} 张 (已完成: {len(done_image_paths)})")

    # ── 分块处理 ──────────────────────────────────────────────────────────────
    pipeline = OptimizedQAPipeline(api_key, concurrency=concurrency)
    ...
    for chunk_idx in range(total_chunks):
        ...
        t0 = time.time()
        chunk_img_qa, chunk_demoted = await pipeline.process_batch(
            chunk,
            n_qa_per_image=n_qa_per_image,
            include_silver=include_silver
        )
        elapsed = time.time() - t0
        processed_so_far = len(done_image_paths) + (chunk_idx + 1) * chunk_size
        pct = min(processed_so_far, len(caption_images)) / len(caption_images) * 100
        remaining_imgs = len(remaining) - (chunk_idx + 1) * chunk_size
        eta_s = elapsed / max(len(chunk), 1) * max(remaining_imgs, 0)
        print(f"  Chunk {chunk_idx+1}/{total_chunks} ({len(chunk)} imgs) "
              f"→ img={len(chunk_img_qa)} txt={len(chunk_demoted)} "
              f"| {elapsed:.0f}s | {pct:.1f}% ETA {eta_s/60:.0f}min", flush=True)

        with open(output_path, 'a') as f:
            for qa in chunk_img_qa:
                f.write(json.dumps(qa, ensure_ascii=False) + '\n')
        with open(text_demoted_path, 'a') as f:
            for qa in chunk_demoted:
                f.write(json.dumps(qa, ensure_ascii=False) + '\n')

        done_image_paths.update(img['image_path'] for img in chunk)
        with open(ckpt_path, 'w') as f:
            json.dump({
                'done_image_paths': list(done_image_paths),
                'usage': USAGE.to_dict(),
            }, f)

    # ── 最终统计 ──────────────────────────────────────────────────────────────
    image_qa_results = [json.loads(l) for l in open(output_path) if l.strip()]
    text_demoted_all = [json.loads(l) for l in open(text_demoted_path) if l.strip()]

    print(f"\n✅ 完成。输出: {output_path}")
    stats = {
        'total': len(image_qa_results),
        'text_demoted': len(text_demoted_all),
    ...
    print(f"  降级为文本QA: {stats['text_demoted']} 条")
    ...
    return image_qa_results, text_demoted_all
...
def main():
    ...
    parser.add_argument('--concurrency', type=int, default=20, help='DeepSeek API 并发数（默认20）')
    parser.add_argument('--chunk-size', type=int, default=500, help='每块图像数（用于进度输出和 checkpoint）')
    parser.add_argument('--papers', default='', help='逗号分隔的 paper_id 列表，只处理这些论文的图像（默认全部）')
    parser.add_argument('--papers-file', default='', help='每行一个 paper_id 的文件，优先于 --papers')
    ...
    if args.papers_file and Path(args.papers_file).exists():
        paper_ids = {l.strip() for l in open(args.papers_file) if l.strip()}
    elif args.papers:
