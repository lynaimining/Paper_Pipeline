...
def step_qwen_vl(corpus_dir: str, qwen_out: str, qwen_model: str,
    ...
    if Path(results_path).exists():
        ...
        print(f"[Step 3] 已有 Qwen VL 结果 ({existing} 条)，跳过推理: {results_path}")
        return results_path
...
def step_pipeline_final(trusted_json: str, text_qa: str, output_dir: str,
    ...
    if paper_ids:
        ...
        # 从 trusted.json 推导所有 paper_id
        all_pids = [str(r["paper_id"]) for r in json.loads(Path(trusted_json).read_text())]
        cmd += ["--paper-ids"] + all_pids

    if eg_root:
        cmd += ["--eg-root", eg_root]
    if image_qa and Path(image_qa).exists():
        # 图 QA 已经生成，直接传路径给 pipeline_final，不需要 --with-image-qa
